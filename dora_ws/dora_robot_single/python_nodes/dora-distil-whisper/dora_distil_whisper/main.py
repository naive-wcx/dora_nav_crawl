...
def load_model():
    """Load whisper model with offline support."""
    from transformers import AutoModelForSpeechSeq2Seq, AutoProcessor, pipeline

    # 检查是否存在本地缓存路径
    local_snapshot_path = "/home/dora/.cache/huggingface/hub/models--openai--whisper-large-v3-turbo/snapshots/41f01f3fe87f28c78e2fbf8b568835947dd65ed9"
    
    # 优先使用环境变量中的模型路径，如果没有则使用本地缓存路径（如果存在），否则使用默认路径
    model_name_or_path = os.getenv("MODEL_NAME_OR_PATH")
    if not model_name_or_path:
        if Path(local_snapshot_path).exists():
            model_name_or_path = local_snapshot_path
            print(f"[INFO] 使用本地缓存模型: {model_name_or_path}")
        else:
            model_name_or_path = DEFAULT_PATH
            print(f"[INFO] 使用默认模型: {model_name_or_path}")
    ...
    # 添加 local_files_only=True 参数以强制离线加载
    model = AutoModelForSpeechSeq2Seq.from_pretrained(
        model_name_or_path,
        torch_dtype=torch_dtype,
        low_cpu_mem_usage=True,
        use_safetensors=True,
        local_files_only=True,  # 强制使用本地文件，不检查在线更新
    )
    model.to(device)
