...
# 调整参数以降低VAD灵敏度
MIN_SILENCE_DURATION_MS = int(os.getenv("MIN_SILENCE_DURATION_MS", "300"))  # 增加到300ms
MIN_SPEECH_DURATION_MS = int(os.getenv("MIN_SPEECH_DURATION_MS", "500"))    # 增加到500ms
THRESHOLD = float(os.getenv("THRESHOLD", "0.92"))                           # 增加到0.92
MAX_AUDIO_DURATION_S = float(os.getenv("MAX_AUDIO_DURATION_S", "75"))
MIN_AUDIO_SAMPLING_DURATION_MS = int(os.getenv("MIN_AUDIO_SAMPLING_DURATION_MS", "2000"))
    ...
    while True:
        ...
        if event["type"] == "INPUT" and event["id"] == "audio":
            ...
            if (
                ...
                #     )
                #     print("send timestamp_start end")
                #     continue
