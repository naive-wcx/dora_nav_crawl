...
void vehicle_stat_pub(void* dora_context);
...
bool analy_uart_recive_data(string serial_data)
{
	...
	for(int k=0;k<header_count;k++) 
	{
		...
		{//检查帧头帧尾是否完整
			...
			else if(reviced_tem[3+step] ==0xAD)
			{
			
				i=4+step;
				uint8_t a = reviced_tem[i++];
				vehicle_stat = reviced_tem[i++]; 
				//if(b==3)手动驾驶
				//if(b==1)自动驾驶
				uint8_t c = reviced_tem[i++];  
			}
			else
			{
...
void vehicle_stat_pub(void* dora_context)
{
	std::string out_id = "vehicle_stat";
	// 创建一个字符数组来存储转换后的字
	// cout<<"开始"<<endl;
	// cout<<static_cast<int>(vehicle_stat)<<endl;
    char stat[2];
    stat[0] = vehicle_stat + '0'; // 将数字转换为字符
    stat[1] = '\0'; // 添加字符串结束符
	// int result = dora_send_output(dora_context, &out_id[0], out_id.length(), stat ,std::strlen(stat));
	// if (result != 0)
	// {
	// 	std::cerr << "failed to send output" << std::endl;
	// }
