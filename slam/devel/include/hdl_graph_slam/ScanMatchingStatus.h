...
struct Printer< ::hdl_graph_slam::ScanMatchingStatus_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::hdl_graph_slam::ScanMatchingStatus_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "prediction_labels: ";
    if (v.prediction_labels.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.prediction_labels.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::std_msgs::String_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.prediction_labels[i]);
    }
    if (v.prediction_labels.empty() || false)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "prediction_errors: ";
    if (v.prediction_errors.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.prediction_errors.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Pose_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.prediction_errors[i]);
    }
    if (v.prediction_errors.empty() || false)
      s << "]";
  }
